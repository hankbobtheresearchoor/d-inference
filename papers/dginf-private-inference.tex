...
RDMA over Thunderbolt~5 (80\,Gb/s) & Disabled by default; requires Recovery OS boot to enable; RDMA status reported in attestation challenge-response (Section~\ref{sec:challenge}) & Architecture + Hardware \\
\midrule
...
\begin{itemize}[leftmargin=*,itemsep=1pt]
    ...
    \item \textbf{Single-node, RDMA enabled, hypervisor active}: Stage~2 page tables isolate inference memory from RDMA. Provides hardware-level defense-in-depth.
    \item \textbf{Single-node, RDMA enabled, no hypervisor}: RDMA status is reported to consumers via attestation; consumers may select trust tiers or routing preferences based on this information. The provider is not automatically marked untrusted.
    \item \textbf{Multi-node (future)}: Both nodes must have RDMA enabled. Hypervisor isolation is recommended. Activations transferred via encrypted RDMA (Section~\ref{sec:future}).
\end{itemize}
...
\texttt{rdmaDisabled} & bool & RDMA status via \texttt{rdma\_ctl} (informational) \\
...
\begin{enumerate}[leftmargin=*,itemsep=1pt]
    ...
    \item Check current hypervisor status (optional)
...
\begin{itemize}[leftmargin=*,itemsep=1pt]
    ...
    \item RDMA and hypervisor status are reported to consumers for trust tier selection; RDMA enabled without hypervisor is not an automatic untrust condition
\end{itemize}
...
DMA/RDMA isolation & Facility controls & IOMMU + optional Hypervisor Stage~2 pages \\
